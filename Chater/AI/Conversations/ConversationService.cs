...
public sealed class ConversationService(ConversationRepository conversations)
{
    ...
    public async Task<Conversation> CreateAsync(ApiProvider provider, Skill? skill, CancellationToken cancellationToken = default)
    {
        ...
        var configuration = JsonSerializer.Serialize(new ProviderSnapshot(provider.ProviderType, provider.ModelId, provider.Endpoint, skill?.SystemPrompt), AI.ChaterJsonSerializerContext.Default.ProviderSnapshot);
    ...
    {
        var configuration = JsonSerializer.Deserialize(conversation.ProviderConfiguration, AI.ChaterJsonSerializerContext.Default.ProviderSnapshot) ?? throw new InvalidOperationException("Conversation provider snapshot is invalid.");
    ...
    public static SessionSnapshot GetRequestedSessionSnapshot(Conversation conversation, ApiProvider provider)
    {
        ...
        var currentConfiguration = JsonSerializer.Serialize(new ProviderSnapshot(provider.ProviderType, model, provider.Endpoint, configuration.SystemPrompt), AI.ChaterJsonSerializerContext.Default.ProviderSnapshot);
